...
public class MyRenderer implements GLSurfaceView.Renderer {
    ...
    public MyRenderer() {
        ...
        ByteBuffer buffer = ByteBuffer.allocateDirect(triangleVerticesData.length * mBytesPerFloat);
        mTriangleVertices = buffer.order(ByteOrder.nativeOrder()).asFloatBuffer();
    ...
    private void drawTriangle(final FloatBuffer aTriangleBuffer) {
        ...
        GLES20.glVertexAttribPointer(mPositionHandle, mPositionDataSize, GLES20.GL_FLOAT, false, mStrideBytes,
                aTriangleBuffer); // ポインタと座標属性を結び付ける
        GLES20.glEnableVertexAttribArray(mPositionHandle); // 座標属性有効

        aTriangleBuffer.position(mColorOffset); // 頂点バッファを色属性にセット
        GLES20.glVertexAttribPointer(mColorHandle, mColorDataSize, GLES20.GL_FLOAT, false, mStrideBytes,
